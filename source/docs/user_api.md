...
Below is a test application that utilizes the Omnitrace API to profile a simple Fibonacci calculation.
...
The test application, `user-api.cpp`, is then compiled using CMake or g++ command.
...
```bash
g++ -g -I/opt/omnitrace/include -L/opt/omnitrace/lib user-api.cpp -o user-api -lomnitrace-user
```

### User API Output

The test application is first instrumented, and then run.
...
Now, we can review the wall_clock output.
