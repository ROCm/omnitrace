...
class line_speedup(object):
    def __init__(self, _name="", _exp_data=None, _exp_base=None):
        self.name = _name
        self.data = _exp_data
        self.base = _exp_base
    ...
    def __str__(self):
        ...
        return f"[{self.name}][{self.data.speedup:3}] speedup: {_line_speedup:6.1f} %"

    def __eq__(self, rhs):
        return self.name == rhs.name and self.data == rhs.data and self.base == rhs.base
        ...
        for exp in record["experiments"]:
            ...
            for pts in exp["progress_points"]:
                ...
                if "delta" in pts:
                    _delt = pts["delta"]
                    _arrive = pts["arrival"]
                    _depart = pts["departure"]
                    if _delt > 0:
                        _weight = _delt if _delt > _weight else _weight
                    _delt = _arrive - _depart
                    if _delt > 0:
                        _weight = _delt if _delt > _weight else _weight
                else:
                    _delt = pts["laps"]
                    _weight = _delt if _delt > _weight else _weight
            itr = find_or_insert(data[_selected], _speedup)
            itr += float(_duration) / float(_weight)
...
def compute_speedups(_data):
    data = {}
    _names = []
    for selected, ditr in _data.items():
        data[selected] = OrderedDict(sorted(ditr.items()))
        if ":" in selected:
            _name = os.path.dirname(selected.split(":")[0])
            if _name not in _names:
                _names.append(_name)

    lcp = longest_common_prefix(_names)

    ret = []
    for selected, ditr in data.items():
        if 0 not in ditr.keys():
            print(f"missing baseline data for {selected}...")
            continue
        selected = selected.replace(lcp, "", 1) if len(lcp) > 0 else selected
        ...
        for speedup, itr in ditr.items():
            ...
            _val = line_speedup(selected, itr, ditr[0])
            ret.append(_val)

    _last_name = None
    for itr in ret:
        if itr.name != _last_name:
            print("")
        print(f"{itr}")
        _last_name = itr.name
